...
def get_column_id_by_name(column_name):
    ...
    if response.status_code == 404:
        print("❌ Project not found or not accessible. This might be a new GitHub Projects (beta) project.")
        print("💡 For new Projects, issues are added automatically when linked, not via API.")
        return None
    if response.ok:
...
def add_issue_to_project(issue_number, column_name):
    ...
    print(f"ℹ️ Skipping project integration for Issue #{issue_number} - using new Projects API")
    return
